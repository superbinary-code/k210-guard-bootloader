...
volatile struct otp_t *const otp RODATA =
	(volatile struct otp_t *)OTP_BASE_ADDR;
...
enum otp_status_t otp_blank_check(void)
{
	...
	if (otp_func_reg_disable_get(BLANK_TEST_DISABLE) == OTP_WIRED_REG_SET)
		return OTP_WIRED_REG_SET;
	...
	if (status != OTP_OK)
		return status;
	status = otp_func_reg_disable_refresh(BLANK_TEST_DISABLE);
	if (status != OTP_OK)
		return status;
	return OTP_OK;
}
...
static enum otp_status_t otp_write_byte(uint32_t addr, uint8_t *data_buf,
					uint32_t length)
{
	enum otp_status_t status;
	uint8_t data, index;
...
static enum otp_status_t otp_read_byte(uint32_t addr, uint8_t *data_buf,
				       uint32_t length)
{
	uint32_t time_out;
...
enum otp_status_t otp_write_data(uint32_t addr, uint8_t *data_buf,
				 uint32_t length)
{
	enum otp_status_t status;
	...
	if (addr >= OTP_BISR_DATA_ADDR)
		return OTP_ERROR_ADDRESS;
	length = length <= OTP_BISR_DATA_ADDR - addr ?
			 length :
			 OTP_BISR_DATA_ADDR - addr;
...
enum otp_status_t otp_read_data(uint32_t addr, uint8_t *data_buf,
				uint32_t length)
{
	enum otp_status_t status;
...
enum otp_status_t otp_key_compare(uint8_t *data_buf)
{
	...
	otp->otp_cmp_key = ((uint32_t)data_buf[0] << 24) |
			   ((uint32_t)data_buf[1] << 16) |
			   ((uint32_t)data_buf[2] << 8) | (uint32_t)data_buf[3];
	otp->otp_cmp_key = ((uint32_t)data_buf[4] << 24) |
			   ((uint32_t)data_buf[5] << 16) |
			   ((uint32_t)data_buf[6] << 8) | (uint32_t)data_buf[7];
	otp->otp_cmp_key =
		((uint32_t)data_buf[8] << 24) | ((uint32_t)data_buf[9] << 16) |
		((uint32_t)data_buf[10] << 8) | (uint32_t)data_buf[11];
	otp->otp_cmp_key = ((uint32_t)data_buf[12] << 24) |
			   ((uint32_t)data_buf[13] << 16) |
			   ((uint32_t)data_buf[14] << 8) |
...
enum otp_status_t otp_data_block_protect_get(enum otp_data_block_t block)
{
	if (block < DATA_BLOCK_MAX / 2) {
		...
		if (otp->block_flag_high &
		    (0x01 << (block - DATA_BLOCK_MAX / 2)))
			return OTP_BLOCK_PROTECTED;
	} else
		return OTP_ERROR_PARAM;
...
enum otp_status_t otp_func_reg_disable_refresh(enum otp_func_reg_t func)
{
	uint8_t value;

	if (func < FUNC_REG_MAX)
		return otp_read_byte(OTP_WIRED_REG_ADDR + func / 4, &value, 1);
	else
		return OTP_ERROR_PARAM;
}

enum otp_status_t otp_soft_write(uint32_t addr, uint8_t *data_buf,
				 uint32_t length)
{
...
enum otp_status_t otp_soft_read(uint32_t addr, uint8_t *data_buf,
				uint32_t length)
{
	otp->otp_ceb_cpu = 1;
	otp->otp_dle_cpu = 0;
