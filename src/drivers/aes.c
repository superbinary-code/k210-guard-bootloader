...
volatile struct aes_t *const aes RODATA =
	(volatile struct aes_t *)AES_BASE_ADDR;
	     ...
	     uint32_t add_size, uint32_t data_size)
{
	...
	for (i = 0; i < num; i++)
		aes->aes_key[i] =
			*((uint32_t *)(&key_addr[key_length - (4 * i) - 4]));
	...
	for (i = 0; i < num; i++)
		aes->aes_iv[i] =
			*((uint32_t *)(&aes_iv[iv_length - (4 * i) - 4]));
	...
	aes->cipher_mode = cipher_mod;
	/*
	 * [1:0],set the first bit and second bit 00:ecb; 01:cbc;
	 * 10,11：aes_gcm
	 */
	aes->encrpt_sel = encrpt_sel;
...
int aes_process_less_80_bytes(uint8_t *aes_in_data, uint8_t *aes_out_data,
			      uint32_t data_size,
			      enum aes_cipher_mod cipher_mod)
{
	...
	// get data
	for (i = 0; i < num; i++) {
		while (!get_data_out_flag())
			;
		*((uint32_t *)(&aes_out_data[i * 4])) = read_out_data();
	}
	if ((cipher_mod == AES_GCM) && (remainder)) {
		while (!get_data_out_flag())
			;
		//u32data = read_out_data();
		*((uint32_t *)(&u8data[0])) = read_out_data();
		switch (remainder) {
		case 1:
		...
		enum aes_cipher_mod cipher_mod)
{
	...
	if (data_size >= 80) {
		for (i = 0; i < (data_size / 80); i++)
			aes_process_less_80_bytes(&aes_in_data[i * 80],
						  &aes_out_data[i * 80], 80,
						  cipher_mod);
	}
	temp_size = data_size % 80;
	if (temp_size)
		aes_process_less_80_bytes(&aes_in_data[i * 80],
					  &aes_out_data[i * 80], temp_size,
					  cipher_mod);
	return 1;
}
