...
int plic_irq_enable(plic_irq_t irq_number)
{
	...
	uint32_t current =
		plic->target_enables.target[core_id].enable[irq_number / 32];
...
void __attribute__((weak, alias("plic_irq_unregister")))
plic_irq_deregister(plic_irq_t irq);

/*Entry Point for PLIC Interrupt Handler*/
uintptr_t __attribute__((weak)) handle_irq_m_ext(uintptr_t cause, uintptr_t epc)
{
	/*
