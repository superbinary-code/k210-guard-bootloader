...
int clint_timer_set_interval(uint64_t interval)
{
	...
	clint_timer_instance[core_id].cycles =
		interval * clint_timer_get_freq() / 1000ULL;
	return 0;
}
...
uintptr_t handle_irq_m_timer(uintptr_t cause, uintptr_t epc)
{
	...
	if (!clint_timer_instance[core_id].single_shot &&
	    clint_timer_instance[core_id].cycles != 0) {
		/* Set mtimecmp by core id */
		clint->mtimecmp[core_id] +=
			clint_timer_instance[core_id].cycles;
	} else
		clear_csr(mie, MIP_MTIP);
	return epc;
...
uintptr_t handle_irq_m_soft(uintptr_t cause, uintptr_t epc)
{
	...
	if (clint_ipi_instance[core_id].callback != NULL)
		clint_ipi_instance[core_id].callback(
			clint_ipi_instance[core_id].ctx);
	clear_csr(mstatus, MSTATUS_MIE);
	set_csr(mstatus, MSTATUS_MPIE | MSTATUS_MPP);
	set_csr(mie, MIP_MSIP);
	return epc;
