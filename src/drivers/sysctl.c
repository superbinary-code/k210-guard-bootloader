...
static uint32_t sysctl_pll_source_set_freq(sysctl_pll_t pll,
					   sysctl_clock_source_t source,
					   uint32_t freq)
{
	uint32_t freq_in = 0;
	...
	for (nfi = val; nfi < nf_max; ++nfi) {
		...
		if ((first) || (fabs(err) < merr * (1 + 1e-6)) ||
			...
			for (no = (not > no_max) ? no_max : not; no > no_min;
			     --no) {
...
uint32_t sysctl_clock_get_freq(sysctl_clock_t clock)
{
	...
	case SYSCTL_CLOCK_CPU:
		...
		case 0:
			...
			break;
		case 1:
			source = sysctl_clock_source_get_freq(
					 SYSCTL_SOURCE_PLL0) /
				 (2ULL << sysctl_clock_get_threshold(
	...
	case SYSCTL_CLOCK_SRAM0:
		...
		result = source /
			 (sysctl_clock_get_threshold(SYSCTL_THRESHOLD_SRAM0) +
			  1);
		break;
	case SYSCTL_CLOCK_SRAM1:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_ACLK);
		result = source /
			 (sysctl_clock_get_threshold(SYSCTL_THRESHOLD_SRAM1) +
			  1);
		break;
	case SYSCTL_CLOCK_ROM:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_ACLK);
		result = source /
			 (sysctl_clock_get_threshold(SYSCTL_THRESHOLD_ROM) + 1);
		break;
	...
	case SYSCTL_CLOCK_APB0:
		...
		result =
			source /
			(sysctl_clock_get_threshold(SYSCTL_THRESHOLD_APB0) + 1);
		break;
	case SYSCTL_CLOCK_APB1:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_ACLK);
		result =
			source /
			(sysctl_clock_get_threshold(SYSCTL_THRESHOLD_APB1) + 1);
		break;
	case SYSCTL_CLOCK_APB2:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_ACLK);
		result =
			source /
			(sysctl_clock_get_threshold(SYSCTL_THRESHOLD_APB2) + 1);
		break;
	...
	case SYSCTL_CLOCK_SPI0:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_PLL0);
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_SPI0) +
			   1) *
			  2);
		break;
	case SYSCTL_CLOCK_SPI1:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_PLL0);
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_SPI1) +
			   1) *
			  2);
		break;
	case SYSCTL_CLOCK_SPI2:
		source = sysctl_clock_source_get_freq(SYSCTL_SOURCE_PLL0);
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_SPI2) +
			   1) *
			  2);
		break;
	...
	case SYSCTL_CLOCK_SPI3:
		...
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_SPI3) +
			   1) *
			  2);
		break;
	case SYSCTL_CLOCK_TIMER0:
		...
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_TIMER0) +
			   1) *
			  2);
		break;
	case SYSCTL_CLOCK_TIMER1:
		...
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_TIMER1) +
			   1) *
			  2);
		break;
	case SYSCTL_CLOCK_TIMER2:
		...
		result = source /
			 ((sysctl_clock_get_threshold(SYSCTL_THRESHOLD_TIMER2) +
			   1) *
			  2);
		break;
...
void sysctl_set_power_mode(sysctl_power_bank_t power_bank,
...
uint32_t sysctl_pll_set_freq(sysctl_pll_t pll, uint32_t pll_freq)
{
	...
	uint32_t result;
	if (pll == SYSCTL_PLL2)
		result = sysctl_pll_source_set_freq(pll, v_pll_t->pll_ckin_sel,
						    pll_freq);
	else
		result = sysctl_pll_source_set_freq(pll, SYSCTL_SOURCE_IN0,
						    pll_freq);
...
uint32_t sysctl_cpu_set_freq(uint32_t freq)
{
	...
	return sysctl_pll_set_freq(SYSCTL_PLL0,
				   (sysctl->clk_sel0.aclk_divider_sel + 1) * 2 *
					   freq);
}
