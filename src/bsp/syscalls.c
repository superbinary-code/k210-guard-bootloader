...
static int sys_gettimeofday(struct timeval *tp, void *tzp)
{
	...
	if (tp != NULL) {
		uint64_t clint_usec = clint->mtime /
				      (sysctl_clock_get_freq(SYSCTL_CLOCK_CPU) /
				       CLINT_CLOCK_DIV / 1000000UL);
	     ...
	     uintptr_t fregs[32])
{
	...
	static uintptr_t (*const syscall_table[])(long a0, long a1, long a2,
		       ...
		       uintptr_t fregs[32])
{
	...
	LOGV(TAG,
	     "misaligned load recovered at %08lx. len:%02d,addr:%08lx,reg:%02d,data:%016lx,signed:%1d,float:%1d",
	     (uint64_t)epc, len, (uint64_t)addr, dst, data_load, !unsigned_,
	     fpu);
			...
			uintptr_t fregs[32])
{
	...
	LOGV(TAG,
	     "misaligned store recovered at %08lx. len:%02d,addr:%08lx,reg:%02d,data:%016lx,float:%1d",
