...
#ifndef __INCLUDE_DUMP_H_
#define __INCLUDE_DUMP_H_
...
static inline void dump_core(const char *reason, uintptr_t cause, uintptr_t epc,
	...
	if (CONFIG_LOG_LEVEL >= LOG_ERROR) {
		...
		for (i = 0; i < 32 / 2; i++) {
			DUMP_PRINTF(
				...
				(float)fregs[i * 2], i * 2 + 1,
				regf_usage[i * 2 + 1][0], fregs[i * 2 + 1],
				(float)fregs[i * 2 + 1]);
		}
	}
}
