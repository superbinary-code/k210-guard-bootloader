...
#define kendryte_replbits_byte(dest, msk, src)                                 \
	(kendryte_write_byte(dest, (kendryte_read_byte(dest) & ~(msk)) |       \
					   ((src) & (msk))))
...
#define kendryte_replbits_hword(dest, msk, src)                                \
	(kendryte_write_hword(dest, (kendryte_read_hword(dest) & ~(msk)) |     \
					    ((src) & (msk))))
...
#define kendryte_replbits_word(dest, msk, src)                                 \
	(kendryte_write_word(dest, (kendryte_read_word(dest) & ~(msk)) |       \
					   ((src) & (msk))))
...
#define kendryte_replbits_dword(dest, msk, src)                                \
	(kendryte_write_dword(dest, (kendryte_read_dword(dest) & ~(msk)) |     \
					    ((src) & (msk))))

#define configASSERT(x)                                                        \
	if ((x) == 0) {                                                        \
		printk("(%s:%d) %s", __FILE__, __LINE__, #x);                  \
		for (;;)                                                       \
			;                                                      \
	}
 ...
 * @param[in]   value       The value to set
 */
void set_bit_offset(volatile uint32_t *bits, uint32_t mask, size_t offset,
		    uint32_t value);
